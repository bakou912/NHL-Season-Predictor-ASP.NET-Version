...
namespace NHLPredictorASP.Classes.Utility
{
    public static class SeasonCalculator
    {
        ...
        ///     <param name="nbSeasons">Integer representing the number of seasons to calculate for the calibration</param>
        /// </summary>
        public static void CalibrateCalculation(int nbSeasons)
        {
            ...
            //Adjustment is meant to bring the expected points of all players to the actual points scored
            Adjustment = totalPoints / totalExpectedPoints;
        }

        /// <summary>
        ///     Calculates an estimation of the player's next season by computing a weighted average with the most important season
        ///     being the most recent
        /// </summary>
        public static void CalculateExpectedSeason(Player player)
        {
            var growthRate =
                1.0; //Growth rate used to adjust the prediction according to the player's production's improvement
            ...
            {
                if (player.SeasonList.Count > 5
                ) //If there are enough seasons to eliminate the ones below games played average
                {
                    if (player.SeasonList[i].GamesPlayed >= 50) //If above respectable number of games played
                    {
                        ...
                        if (previousValid)
                        {
                            weightsList[weightsList.Count - 1] *=
                                (double) player.SeasonList[i].Points / player.SeasonList[i - 1].Points;
                        }
